...
async def cmd_logout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if handler and sessions:
        ...
        async with sessions.session(chat_id) as session:
            if session and session.is_authenticated:
                await sessions.logout(chat_id)
                await handler.send_message(chat_id, "👋 با موفقیت از حساب کاربری خارج شدید.")
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sessions:
        ...
        await context.bot.send_message(
            ...
            f"نرخ موفقیت کش: {stats.get('cache_hit_rate', 0):.2%}\n"
        )
