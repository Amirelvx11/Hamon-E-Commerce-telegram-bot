...
from .CoreConfig import (
    ...
    STEP_ICONS, get_step_info, get_step_display, COMPLAINT_TYPE_MAP,
...
class CallbackHandler:
    ...
    async def _route_callback(self, query: CallbackQuery, chat_id: int, message_id: int, data: str, session):
        ...
        if data in [CallbackFormats.MAIN_MENU, "main_menu"]:
            ...
            order_number = self._extract_order_number(data)
            await self.handle_devices_list(chat_id, message_id, order_number, session, page=1)
        elif data.startswith("page_") and "devices" in data:
            page, order_number = self._extract_device_page(data)
            await self.handle_devices_list(chat_id, message_id, order_number, session, page)
    ...
    async def _show_error(self, chat_id: int, message_id: int, error_msg: str = None):
        ...
        try:
            await self.msg.edit_message(chat_id, message_id, text, reply_markup=keyboard, activate_keyboard=True)
        except Exception as e:
    ...
    async def _require_auth(self, chat_id: int, message_id: int, session):
        ...
        await self.msg.edit_message(
            chat_id, message_id, "⚠️ ابتدا وارد حساب کاربری شوید",
            reply_markup=keyboard, activate_keyboard=True
        )
    ...
    async def handle_main_menu(self, chat_id: int, message_id: int, session):
        ...
        try:
            ...
            if session.is_authenticated and session.nationalId:
                ...
                await self._show_authenticated_menu(chat_id, message_id, session.user_name)
            else:
    ...
    async def handle_cancel(self, chat_id: int, message_id: int, session):
        ...
        try:
            ...
            try:
                await self.msg.edit_message(
                    chat_id, message_id, "❌ عملیات لغو شد",
                    reply_markup=None, activate_keyboard=False
                )
    ...
    async def handle_logout(self, chat_id: int, message_id: int, session):
        ...
        try:
            ...
            await self.msg.edit_message(
                chat_id, message_id, "✅ با موفقیت خارج شدید",
                reply_markup=None, activate_keyboard=True
            )
            await asyncio.sleep(1)
            await self._show_main_menu(chat_id, message_id)
        except Exception as e:
...
📊 **وضعیت حساب**: فعال ✅"""
    ...
    async def handle_my_orders(self, chat_id: int, message_id: int, session, page: int = 1):
        ...
        try:
            await self.msg.edit_message(
                chat_id, message_id, "⏳ در حال دریافت سفارشات...",
            ...
            # Pagination
            total_pages = (len(orders) + self.ORDERS_PER_PAGE - 1) // self.ORDERS_PER_PAGE
            ...
            nav_row = []
            if page > 1:
                nav_row.append(InlineKeyboardButton("⬅️ قبلی", callback_data=f"my_orders_page_{page-1}"))
            nav_row.append(InlineKeyboardButton(f"📄 {page}/{total_pages}", callback_data="noop"))
            if page < total_pages:
                nav_row.append(InlineKeyboardButton("➡️ بعدی", callback_data=f"my_orders_page_{page+1}"))
            ...
            if total_pages > 3:
                quick_nav = []
                if page > 2:
                    quick_nav.append(InlineKeyboardButton("⏮ اول", callback_data="my_orders_page_1"))
                if page < total_pages - 1:
                    quick_nav.append(InlineKeyboardButton("⏭ آخر", callback_data=f"my_orders_page_{total_pages}"))
                if quick_nav:
            ...
            await self.msg.edit_message(
                ...
                parse_mode=ParseMode.MARKDOWN, activate_keyboard=True
            )
    ...
    async def handle_track_by_number(self, chat_id: int, message_id: int, session):
        ...
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 انصراف", callback_data=CallbackFormats.MAIN_MENU)]])
    ...
    async def handle_order_details(self, chat_id: int, message_id: int, order_number: str, session):
        ...
        try:
            # Show loading
            await self.msg.edit_message(
                chat_id, message_id, "⏳ دریافت جزئیات سفارش...",
                activate_keyboard=False
            )
...
• کل هزینه: {order_info.total_cost:,} تومان"""
    ...
    async def handle_refresh_order(self, chat_id: int, message_id: int, order_number: str, session):
        ...
        try:
            # Show loading
            await self.msg.edit_message(
                chat_id, message_id, "🔄 در حال بروزرسانی...",
                activate_keyboard=False
            )
            
            # Fetch fresh data (force refresh)
            fresh_order = await self.data.get_order(order_number, "number", force_refresh=True)
            ...
            buttons = [
                [InlineKeyboardButton("🔄 بروزرسانی مجدد", callback_data=f"refresh_order:{order_number}")],
                [InlineKeyboardButton("🔙 بازگشت", callback_data=CallbackFormats.MAIN_MENU)]
            ]
            ...
            if fresh_order.payment_link and not fresh_order.factor_payment:
                buttons.insert(0, [
                    InlineKeyboardButton("💳 پرداخت فاکتور", url=fresh_order.payment_link)
                ])
    ...
    async def handle_devices_list(self, chat_id: int, message_id: int, order_number: str, session, page: int = 1):
        ...
        try:
            ...
            if not order_info or not order_info.devices:
                await self._show_error(chat_id, message_id, "❌ اطلاعات دستگاه یافت نشد")
                return
            ...
            if total_pages > 1:
                nav_row = []
                if page > 1:
                    nav_row.append(InlineKeyboardButton("⬅️ قبلی", callback_data=f"page_{page-1}_devices_{order_number}"))
                nav_row.append(InlineKeyboardButton(f"📱 {page}/{total_pages}", callback_data="noop"))
                if page < total_pages:
                    nav_row.append(InlineKeyboardButton("➡️ بعدی", callback_data=f"page_{page+1}_devices_{order_number}"))
                buttons.append(nav_row)

            buttons.append([
                InlineKeyboardButton("🔙 بازگشت", callback_data=f"order_{order_number}")
            ])
...
لطفاً نوع موضوع را انتخاب کنید تا بتوانیم بهتر به شما کمک کنیم:"""
        ...
        complaint_keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔧 مشکل فنی", callback_data="complaint_technical")],
            [InlineKeyboardButton("💰 مسائل مالی", callback_data="complaint_payment")],
            [InlineKeyboardButton("📦 ارسال/تحویل", callback_data="complaint_shipping")],
            [InlineKeyboardButton("🎧 پشتیبانی", callback_data="complaint_service")],
            [InlineKeyboardButton("📝 سایر موارد", callback_data="complaint_other")],
            [InlineKeyboardButton("❌ انصراف", callback_data=CallbackFormats.MAIN_MENU)]
        ])
        
        await self.msg.edit_message(
            chat_id, message_id, text,
            reply_markup=complaint_keyboard, parse_mode=ParseMode.MARKDOWN,
            activate_keyboard=True
        )
...
حداقل 30 کاراکتر"""
    ...
    async def show_main_menu(self, chat_id: int, message_id: int = None, is_authenticated: bool = False):
    ...
    async def show_help(self, chat_id: int, message_id: int = None):
        ...
        try:
            ...
            if message_id:
                await self.msg.edit_message(chat_id, message_id, help_text, reply_markup=keyboard)
            else:
    ...
    async def _show_welcome_menu(self, chat_id: int, message_id: int = None):
        ...
        keyboard = InlineKeyboardMarkup(MAIN_INLINE_KEYBOARD)
        text = MESSAGES.get('welcome', "🌟 به ربات هامون خوش آمدید!\n\nلطفاً یک گزینه را انتخاب کنید:")
    ...
    async def _show_authenticated_menu(self, chat_id: int, message_id: int, name: str):
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🔧 تعمیرات", callback_data=CallbackFormats.REPAIR_REQUEST),
             InlineKeyboardButton("📝 شکایت", callback_data=CallbackFormats.SUBMIT_COMPLAINT)],
